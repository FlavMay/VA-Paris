...
export function calcMetrics(bien, s) {
  if (!bien?.prix || !bien?.loyerMensuel) return null
  ...
  return {
    lirr: calcIRR(flows), apport: app, credit: cred, mensualite: M,
    travaux: tr, fraisNotaire: fn, totalInvesti: tot, loyerAnnuel: lAn,
    charges: ch, cashflowAnnuel: cf, revente: rev, produitNetRevente: nr,
    capitalRestant: cr,
    rendementBrut: bien.loyerMensuel * 12 / bien.prix * 100,
    rendementNet: (lAn - ch) / tot * 100,
    cashOnCash: cf / app * 100
  }
}

export function calcScore(bien, m, s) {
  if (!m?.lirr) return 0
...
export const ETAT_LABELS = { gros: 'Gros œuvre', complet: 'Rénovation complète', leger: 'Rafraîchissement', occupe: 'Occupé (loué)' }
export const fmt = {
  euro:    (n, d = 0) => n == null ? '—' : n.toLocaleString('fr-FR', { maximumFractionDigits: d }) + '\u00a0€',
  pct:     (n, d = 1) => n == null ? '—' : n.toLocaleString('fr-FR', { maximumFractionDigits: d }) + '\u00a0%',
  pm2:     n          => n == null ? '—' : Math.round(n).toLocaleString('fr-FR') + '\u00a0€/m²',
